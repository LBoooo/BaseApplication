package com.hinacle.base.widget.switchview
...
class SwitchView @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) :
    View(context, attrs) {
    ...
    private fun calcBTranslate(percent: Float): Float {
        var result = 0f
        val switchState = state - lastState
        when (switchState) {
            1 ->                 // off -> off2
     ...
     * if set true , the state change to on;
     * if set false, the state change to off
     * <br></br>**change state with animation**
     *
     * @param isChecked
     */
    ...
    interface OnCheckedChangeListener {
        fun onCheckedChanged(switchView: SwitchView?, isChecked: Boolean)
    }
