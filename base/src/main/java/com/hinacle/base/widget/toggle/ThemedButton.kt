...
import com.hinacle.base.R.styleable.*
...
class ThemedButton : RelativeLayout {
    ...
    var icon: Drawable
        ...
        set(icon) {
            ...
            ivIcon.layoutParams = LayoutParams(80.dp, 80.dp)
        }
    ...
    var selectedIcon: Drawable
        ...
        set(icon) {
            ...
            ivSelectedIcon.layoutParams = LayoutParams(80.dp, 80.dp)
        }

    var borderWidth: Float
        get() = cvCard.borderWidth
        set(value) {
            cvCard.borderWidth = value
        }

    var selectedBorderWidth: Float
        get() = cvSelectedCard.borderWidth
        set(value) {
            cvSelectedCard.borderWidth = value
        }

    var borderColor: Int
        get() = cvCard.borderColor
        set(value) {
            cvCard.borderColor = value
        }

    var selectedBorderColor: Int
        get() = cvSelectedCard.borderColor
        set(value) {
            cvSelectedCard.borderColor = value
        }
    ...
    private fun getStyledAttributes(attributeSet: AttributeSet) {
        ...
        attrs.getColor(ThemedButton_toggle_selectedBackgroundColor, denim)
            .also { this.selectedBgColor = it }
        attrs.getColor(ThemedButton_toggle_textColor, darkGray).also { this.textColor = it }
        attrs.getColor(ThemedButton_toggle_selectedTextColor, context.color(android.R.color.white))
            .also { this.selectedTextColor = it }
        attrs.getBoolean(ThemedButton_toggle_circularCornerRadius, false)
            .also { this.circularCornerRadius = it }
        attrs.getDimension(ThemedButton_toggle_borderWidth, 0F)
            .also { this.borderWidth = it; this.selectedBorderWidth = it }
        attrs.getDimension(ThemedButton_toggle_selectedBorderWidth, this.borderWidth)
            .also { this.selectedBorderWidth = it }
        attrs.getColor(ThemedButton_toggle_borderColor, cvCard.borderColor)
            .also { borderColor = it; selectedBorderColor = it }
        attrs.getColor(ThemedButton_toggle_selectedBorderColor, this.borderColor)
            .also { this.selectedBorderColor = it }
        attrs.getDimension(ThemedButton_toggle_btnCornerRadius, 21F.dp)
            .also { applyToCards { c -> c.cornerRadius = it } }
        attrs.getDimension(ThemedButton_toggle_padding, -1F)
            .also { applyToCards { c -> c.setViewPadding(all = it) } }
        attrs.getDimension(ThemedButton_toggle_paddingHorizontal, -1F)
            .also { applyToCards { c -> c.setViewPadding(horizontal = it) } }
        attrs.getDimension(ThemedButton_toggle_paddingVertical, -1F)
            .also { applyToCards { c -> c.setViewPadding(vertical = it) } }
        attrs.getDimension(ThemedButton_toggle_paddingRight, -1F)
            .also { applyToCards { c -> c.setViewPadding(right = it) } }
        attrs.getDimension(ThemedButton_toggle_paddingTop, -1F)
            .also { applyToCards { c -> c.setViewPadding(top = it) } }
        attrs.getDimension(ThemedButton_toggle_paddingLeft, -1F)
            .also { applyToCards { c -> c.setViewPadding(left = it) } }
        attrs.getDimension(ThemedButton_toggle_paddingBottom, -1F)
            .also { applyToCards { c -> c.setViewPadding(bottom = it) } }
        attrs.getDimension(ThemedButton_toggle_textPadding, -1F)
            .also { applyToTexts { c -> c.setViewPadding(all = it) } }
        attrs.getDimension(ThemedButton_toggle_textPaddingHorizontal, 14F.dp)
            .also { applyToTexts { t -> t.setViewPadding(horizontal = it) } }
        attrs.getDimension(ThemedButton_toggle_textPaddingVertical, -1F)
            .also { applyToTexts { t -> t.setViewPadding(vertical = it) } }
        attrs.getDimension(ThemedButton_toggle_textPaddingRight, -1F)
            .also { applyToTexts { t -> t.setViewPadding(right = it) } }
        attrs.getDimension(ThemedButton_toggle_textPaddingTop, -1F)
            .also { applyToTexts { t -> t.setViewPadding(top = it) } }
        attrs.getDimension(ThemedButton_toggle_textPaddingLeft, -1F)
            .also { applyToTexts { t -> t.setViewPadding(left = it) } }
        attrs.getDimension(ThemedButton_toggle_textPaddingBottom, -1F)
            .also { applyToTexts { t -> t.setViewPadding(bottom = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPadding, -1F)
            .also { applyToIcons { c -> c.setViewPadding(all = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPaddingHorizontal, -1F)
            .also { applyToIcons { i -> i.setViewPadding(horizontal = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPaddingVertical, -1F)
            .also { applyToIcons { i -> i.setViewPadding(vertical = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPaddingRight, -1F)
            .also { applyToIcons { i -> i.setViewPadding(right = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPaddingTop, -1F)
            .also { applyToIcons { i -> i.setViewPadding(top = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPaddingLeft, -1F)
            .also { applyToIcons { i -> i.setViewPadding(left = it) } }
        attrs.getDimension(ThemedButton_toggle_iconPaddingBottom, -1F)
            .also { applyToIcons { i -> i.setViewPadding(bottom = it) } }
        attrs.getDrawable(ThemedButton_toggle_icon)
            ?.let { this.icon = it; this.selectedIcon = it.constantState?.newDrawable()!! }
        ...
        attrs.getInt(ThemedButton_toggle_iconGravity, Gravity.CENTER)
            .also { applyToIcons { i -> i.layoutGravity = it } }
        attrs.getDimension(ThemedButton_toggle_textSize, 15F)
            .also { applyToTexts { t -> t.textSize = it } }
        attrs.getInt(ThemedButton_toggle_textGravity, Gravity.CENTER)
            .also { applyToTexts { i -> i.layoutGravity = it } }
        attrs.getInt(ThemedButton_toggle_textAlignment, 4)
            .also { applyToTexts { t -> t.textAlignment = it } }
