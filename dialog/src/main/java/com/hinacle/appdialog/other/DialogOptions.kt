...
open class DialogOptions() : Parcelable {
    ...
    /**
     * 是否启用防止内存泄露模式
     * 当在activity中使用AppDialog时，如果不是一次性使用，而是被一个对象引用，多次使用
     ...
     */
    var dialogThemeFun: (appDialog: AppDialog) -> Int = {
                ...
                if (this!!.window.decorView.systemUiVisibility == View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
                    ...
                    (findViewById<View>(android.R.id.content) as ViewGroup).getChildAt(0).fitsSystemWindows.let {
                        ...
                        if (it) {//无论是否预留了statusBar，都将dialog设置可占满全屏
                            ...
                            //appDialog.dialog.window.statusBarColor = dialogStatusBarColor
                    ...
                    //appDialog.dialog.window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)
     ...
     */
    fun setDialogTheme(function: (appDialog: AppDialog) -> Int) {
     ...
     */
    fun setStatusMode(function: (appDialog: AppDialog) -> Unit) {
